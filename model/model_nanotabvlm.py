...
class NanoTabVLMConfig(NanoTabLLMConfig):
    model_type = "tabvlm"

    def __init__(
            self,
            image_special_token: str = '@@@@' * 196,
            image_ids: List = [62182] * 196,
            **kwargs,
    ):
...
# 继承自语言模型
class NanoTabVLM(NanoTabLLMForCausalLM):
    config_class = NanoTabVLMConfig

    def __init__(self, params: NanoTabVLMConfig = None, vision_model_path="./model/vision_model/siglip2-base-patch16-512"):
    ...
    def forward(self,
        ...
        if pixel_values is not None and start_pos == 0:
            ...
            vision_tensors = torch.stack([
                NanoTabVLM.get_image_embeddings(pixel_values[:, i, :, :, :], self.vision_encoder)
                for i in range(num)
            ], dim=stack_dim)
